...
function initialize(data) {
  ...
  setDefinitionList(selection, [["Tile", "None"]]);
  resetCamera();
  setStatus(`Loaded ${data.stats.tileCount} tiles from ${dataUrl}. Left-drag pans. Right-drag rotates. WASD/arrows fly.`);
}
...
function initializeMeshes(data) {
  ...
  updateSummary();
  requestDraw();
  setStatus(`Loaded ${state.data.stats.tileCount} tiles and ${data.stats.meshInstanceCount} mesh instances. WASD/arrows fly.`);
}
...
canvas.addEventListener("click", (event) => {
  ...
  updateSelection(pickTile(screenPoint(event)));
});
